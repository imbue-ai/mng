...
def test_prevent_exec_usage() -> None:
    ...
    assert len(chunks) <= snapshot(5), PREVENT_EXEC.format_failure(chunks)
...
def test_prevent_while_true() -> None:
    ...
    assert len(chunks) <= snapshot(3), PREVENT_WHILE_TRUE.format_failure(chunks)
...
def test_prevent_bare_print() -> None:
    ...
    assert len(chunks) <= snapshot(15), PREVENT_BARE_PRINT.format_failure(chunks)
...
def test_prevent_broad_exception_catch() -> None:
    ...
    assert len(chunks) <= snapshot(6), PREVENT_BROAD_EXCEPTION_CATCH.format_failure(chunks)
...
def test_prevent_builtin_exception_raises() -> None:
    ...
    assert len(chunks) <= snapshot(8), PREVENT_BUILTIN_EXCEPTION_RAISES.format_failure(chunks)
...
def test_prevent_inline_imports() -> None:
    ...
    assert len(chunks) <= snapshot(32), PREVENT_INLINE_IMPORTS.format_failure(chunks)
...
def test_prevent_dataclasses_import() -> None:
    ...
    assert len(chunks) <= snapshot(1), PREVENT_DATACLASSES_IMPORT.format_failure(chunks)
...
def test_prevent_trailing_comments() -> None:
    ...
    assert len(chunks) <= snapshot(39), PREVENT_TRAILING_COMMENTS.format_failure(chunks)
...
def test_prevent_monkeypatch_setattr() -> None:
    ...
    assert len(chunks) <= snapshot(6), PREVENT_MONKEYPATCH_SETATTR.format_failure(chunks)
...
def test_prevent_if_elif_without_else() -> None:
    ...
    assert len(chunks) <= snapshot(5), PREVENT_IF_ELIF_WITHOUT_ELSE.format_failure(chunks)


def test_prevent_inline_functions_in_non_test_code() -> None:
    chunks = find_inline_functions(_get_project_source_dir())
    assert len(chunks) <= snapshot(7), PREVENT_INLINE_FUNCTIONS.format_failure(chunks)
...
def test_prevent_init_methods_in_non_exception_classes() -> None:
    ...
    assert len(chunks) <= snapshot(6), PREVENT_INIT_IN_NON_EXCEPTION_CLASSES.format_failure(chunks)
