...
def _build_ssh_activity_wrapper_script(session_name: str, host_dir: Path, agent_command: str) -> str:
    ...
    5. Checks for signal files (written by tmux Ctrl-q/Ctrl-t bindings) and
       exits with a specific code to tell the local mng process what to do
...
def build_ssh_base_args(
    ...
    Raises MngError if no known_hosts file is configured and
    ...
    if ssh_known_hosts_file and ssh_known_hosts_file != "/dev/null":
        ...
        raise MngError("No known_hosts file is configured for this host. Cannot establish a secure SSH connection.")
...
def _determine_post_disconnect_action(
    ...
    if exit_code == SIGNAL_EXIT_CODE_DESTROY:
        return ("mng", ["mng", "destroy", "--session", session_name, "-f"])
    elif exit_code == SIGNAL_EXIT_CODE_STOP:
        return ("mng", ["mng", "stop", "--session", session_name])
    else:
        return None
...
def resolve_connect_command(
    ...
    return mng_ctx.config.connect_command
...
def run_connect_command(
    ...
    """
    env = dict(os.environ)
    env["MNG_AGENT_NAME"] = agent_name
    env["MNG_SESSION_NAME"] = session_name
    env["MNG_HOST_IS_LOCAL"] = "true" if is_local else "false"
...
def connect_to_agent(
    ...
    current process with the appropriate mng command to perform the action locally.
    ...
    if host.is_local:
        ...
        # When tmux_socket_dir is configured, mng uses a separate tmux server,
        # so the existing $TMUX is irrelevant and we always unset it.
        if mng_ctx.config.tmux_socket_dir is not None:
            # Separate tmux server -- $TMUX is irrelevant, always remove it
            env = dict(os.environ)
            env.pop("TMUX", None)
            env["TMUX_TMPDIR"] = str(mng_ctx.config.tmux_socket_dir.expanduser())
            ...
            # Not in tmux, no isolation -- use the current environment as-is
            env = os.environ
