...
class LogsTarget(FrozenModel):
    ...
    def _validate_online_host_and_logs_path_are_paired(self) -> "LogsTarget":
        ...
        if is_host_set != is_path_set:
            raise MngError("online_host and logs_path must both be set or both be None")
        return self
...
def list_log_files(target: LogsTarget) -> list[LogFileEntry]:
    ...
    raise MngError(f"Cannot list log files for {target.display_name}: no volume or online host available")
...
def read_log_content(target: LogsTarget, log_file_name: str) -> str:
    ...
    raise MngError(f"Cannot read log file for {target.display_name}: no volume or online host available")
...
def _read_log_content_via_host(
    ...
    with log_span("Reading log file '{}' for {} via host", log_file_name, display_name):
        ...
        if not result.success:
            raise MngError(f"Failed to read log file '{log_file_name}': {result.stderr}")
...
def follow_log_file(
    ...
    raise MngError(f"Cannot follow log file for {target.display_name}: no volume or online host available")
...
def _follow_log_file_via_host(
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            raise MngError(f"Failed to follow log file (exit code {process.returncode}): {stderr_output.strip()}")
