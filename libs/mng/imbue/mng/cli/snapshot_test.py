...
def test_snapshot_create_help_exits_zero(
    cli_runner: CliRunner,
    plugin_manager: pluggy.PluginManager,
) -> None:
    """Test that `snapshot create --help` works and exits 0."""
    result = cli_runner.invoke(cli, ["snapshot", "create", "--help"])
    assert result.exit_code == 0
    assert "create" in result.output.lower()


def test_snapshot_list_help_exits_zero(
    cli_runner: CliRunner,
    plugin_manager: pluggy.PluginManager,
) -> None:
    """Test that `snapshot list --help` works and exits 0."""
    result = cli_runner.invoke(cli, ["snapshot", "list", "--help"])
    assert result.exit_code == 0
    assert "list" in result.output.lower()


def test_snapshot_destroy_help_exits_zero(
    cli_runner: CliRunner,
    plugin_manager: pluggy.PluginManager,
) -> None:
    """Test that `snapshot destroy --help` works and exits 0."""
    result = cli_runner.invoke(cli, ["snapshot", "destroy", "--help"])
    assert result.exit_code == 0
...
def test_snapshot_create_nonexistent_agent(
    ...
    """Snapshot create with a nonexistent agent should fail with an error message."""
    result = cli_runner.invoke(
        snapshot,
        ["create", "nonexistent-agent-xyz"],
        obj=plugin_manager,
...
def test_snapshot_list_nonexistent_agent(
    cli_runner: CliRunner,
    plugin_manager: pluggy.PluginManager,
) -> None:
    """Snapshot list with a nonexistent agent should fail with an error message."""
    result = cli_runner.invoke(
        snapshot,
        ["list", "nonexistent-agent-xyz"],
        obj=plugin_manager,
        catch_exceptions=True,
    )
    assert result.exit_code != 0
