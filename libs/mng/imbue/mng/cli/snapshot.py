...
def _classify_mixed_identifiers(
    ...
    all_agent_refs = list_agents(mng_ctx, is_streaming=False, error_behavior=ErrorBehavior.CONTINUE).agents
...
def _resolve_snapshot_hosts(
    ...
    for host_str in host_identifiers:
        host_id, provider_name = _find_host_across_providers(host_str, mng_ctx)
...
Useful for checkpointing work, creating restore points, or managing disk space.""",
    aliases=("snap",),
    examples=(
        ("Create a snapshot of an agent's host", "mng snapshot create my-agent"),
