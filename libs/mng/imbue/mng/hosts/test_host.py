...
def test_create_work_dir_copy_with_git_skips_info_exclude_when_disabled(
    host_with_temp_dir: tuple[Host, Path],
    setup_git_config: None,
) -> None:
    """Test that .git/info/exclude is NOT copied when copy_git_info_exclude is False."""
    host, temp_dir = host_with_temp_dir

    source_path = temp_dir / "source_no_info_exclude"
    source_path.mkdir()
    (source_path / "file1.txt").write_text("content")
    _init_git_repo(source_path)
    ...
    target_path = temp_dir / "target_no_info_exclude"

    options = CreateAgentOptions(
        name=AgentName("skip-info-exclude"),
        agent_type=AgentTypeName("generic"),
        command=CommandString("sleep 1"),
        target_path=target_path,
        git=AgentGitOptions(copy_git_info_exclude=False),
    )

    host.create_agent_work_dir(host, source_path, options)

    target_exclude = target_path / ".git" / "info" / "exclude"
    # The default git init exclude file should be present (from git init --bare),
    # but it should NOT contain our custom pattern
    assert "my_custom_pattern" not in target_exclude.read_text()
