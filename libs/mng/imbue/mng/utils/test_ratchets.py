...
def test_no_eager_modal_import() -> None:
    """Importing imbue.mng.main must not eagerly load heavyweight backend or celpy modules.
    ...
    result = subprocess.run(
        [
            sys.executable,
            "-c",
            "import sys; import imbue.mng.main; "
            "heavyweight = ['imbue.mng.providers.modal.backend', "
            "'imbue.mng.providers.docker.backend', "
            "'imbue.mng.providers.ssh.backend', "
            "'imbue.mng.providers.local.backend', "
            "'celpy']; "
