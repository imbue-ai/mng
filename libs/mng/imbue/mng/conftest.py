...
def _get_stale_docker_state_containers(max_age_seconds: int = 3600) -> list[tuple[str, str]]:
    ...
    A state container is considered test-originated if either:
...
def _remove_docker_containers(containers: list[tuple[str, str]]) -> None:
    ...
    Takes a list of (container_id, container_name) tuples. For state containers,
    the Docker named volume has the same name as the container, so we also
    ...
    for container_id, name in containers:
        ...
        # Also remove the backing Docker volume (same name as the container).
        if name:
            try:
