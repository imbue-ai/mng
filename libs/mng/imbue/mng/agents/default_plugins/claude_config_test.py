...
def test_add_claude_trust_creates_config_when_none_exists(tmp_path: Path) -> None:
    ...
    # HOME points to a test-isolated temp dir (autouse setup_test_mng_env)
    config_file = get_claude_config_path()
...
def test_extend_claude_trust_creates_entry_for_worktree(tmp_path: Path) -> None:
    ...
    # Check mng metadata is added
    assert worktree_config["_mngCreated"] is True
    assert worktree_config["_mngSourcePath"] == str(source_path)
...
def test_remove_claude_trust_removes_mng_created_entry(tmp_path: Path) -> None:
    """Test that remove_claude_trust_for_path removes mng-created entries."""
    config_file = get_claude_config_path()
    worktree_path = tmp_path / "worktree"
    worktree_path.mkdir()
...
def test_remove_claude_trust_skips_non_mng_entry(tmp_path: Path) -> None:
    """Test that remove_claude_trust_for_path skips entries not created by mng."""
    config_file = get_claude_config_path()
    worktree_path = tmp_path / "worktree"
    worktree_path.mkdir()
