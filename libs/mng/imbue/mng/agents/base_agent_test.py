...
def test_get_dialog_indicators_returns_empty_by_default(
    local_provider: LocalProviderInstance,
    temp_host_dir: Path,
    temp_work_dir: Path,
) -> None:
    """Test that get_dialog_indicators returns empty tuple by default."""
    test_agent = create_test_agent(local_provider, temp_host_dir, temp_work_dir)
    assert test_agent.get_dialog_indicators() == ()


def test_check_for_blocking_dialog_is_noop_when_no_indicators(
    local_provider: LocalProviderInstance,
    temp_host_dir: Path,
    temp_work_dir: Path,
) -> None:
    """Test that _check_for_blocking_dialog does nothing when no indicators are configured."""
    test_agent = create_test_agent(local_provider, temp_host_dir, temp_work_dir)
    session_name = f"{test_agent.mng_ctx.config.prefix}{test_agent.name}"

    # Create a tmux session that contains dialog-like text
    test_agent.host.execute_command(
        f"tmux new-session -d -s '{session_name}' 'echo \"Do you want to proceed?\"; sleep 847501'",
        timeout_seconds=5.0,
    )
    ...
    try:
        ...
        wait_for(
            lambda: "Do you want to proceed?" in (test_agent._capture_pane_content(session_name) or ""),
            timeout=5.0,
            error_message="Dialog text not visible in pane",
        )

        # Should NOT raise because BaseAgent has no dialog indicators
        test_agent._check_for_blocking_dialog(session_name)
    finally:
        cleanup_tmux_session(session_name)
