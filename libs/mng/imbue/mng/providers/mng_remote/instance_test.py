"""Unit tests for mng remote provider instance."""
...
class FakeClient(MngRemoteClient):
    """Test double for MngRemoteClient that returns canned data."""
...
def _make_provider(
    ...
    should_raise: bool = False,
) -> tuple[MngRemoteProviderInstance, FakeClient]:
    ...
    provider = MngRemoteProviderInstance(
...
class TestListHosts:
    def test_returns_remote_hosts_grouped_by_host(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
        agents = [
        ...
        provider, _ = _make_provider(temp_mng_ctx, fake_agents=agents)

        hosts = provider.list_hosts(cg=temp_mng_ctx.concurrency_group)
    ...
    def test_returns_multiple_hosts(self, temp_mng_ctx: MngContext) -> None:
    ...
    def test_returns_empty_list_when_no_agents(self, temp_mng_ctx: MngContext) -> None:
        provider, _ = _make_provider(temp_mng_ctx, fake_agents=[])

        hosts = provider.list_hosts(cg=temp_mng_ctx.concurrency_group)

        assert hosts == []

    def test_raises_provider_error_when_api_unreachable(self, temp_mng_ctx: MngContext) -> None:
        provider, _ = _make_provider(temp_mng_ctx, should_raise=True)

        with pytest.raises(ProviderError, match="Connection refused"):
            provider.list_hosts(cg=temp_mng_ctx.concurrency_group)

    def test_skips_agents_without_host_id(self, temp_mng_ctx: MngContext) -> None:
        agents = [
...
class TestRemoteHostState:
    def test_passes_through_running_state(self, temp_mng_ctx: MngContext) -> None:
    ...
    def test_passes_through_stopped_state(self, temp_mng_ctx: MngContext) -> None:
    ...
    def test_passes_through_paused_state(self, temp_mng_ctx: MngContext) -> None:
...
class TestRemoteHostAgentReferences:
    def test_returns_agent_references_from_prefetched_data(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
    ...
    def test_returns_multiple_agent_references(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
        agent_1 = _make_agent_data(host_id=host_id, agent_name="agent-1")
        agent_2 = _make_agent_data(host_id=host_id, agent_name="agent-2")
        provider, _ = _make_provider(temp_mng_ctx, fake_agents=[agent_1, agent_2])
    ...
    def test_skips_malformed_agent_records(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
        agents = [
...
class TestGetHost:
    def test_returns_cached_host_by_id(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
    ...
    def test_returns_cached_host_by_name(self, temp_mng_ctx: MngContext) -> None:
    ...
    def test_raises_not_found_when_cache_empty(self, temp_mng_ctx: MngContext) -> None:
        provider, _ = _make_provider(temp_mng_ctx)

        with pytest.raises(HostNotFoundError):
            provider.get_host(HostId(str(HostId())))

    def test_raises_not_found_for_unknown_id(self, temp_mng_ctx: MngContext) -> None:
...
class TestListPersistedAgentData:
    def test_returns_cached_data(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
        agent_id = str(AgentId())
    ...
    def test_fetches_from_api_when_cache_empty(self, temp_mng_ctx: MngContext) -> None:
        host_id = str(HostId())
        agent_id = str(AgentId())
        agents = [_make_agent_data(host_id=host_id, agent_id=agent_id)]
        provider, _ = _make_provider(temp_mng_ctx, fake_agents=agents)
    ...
    def test_raises_when_api_unreachable(self, temp_mng_ctx: MngContext) -> None:
        provider, _ = _make_provider(temp_mng_ctx, should_raise=True)
