...
def test_write_agent_cache_writes_json(tmp_path: Path) -> None:
    """write_agent_cache should write a JSON file with sorted unique names."""
...
def test_write_agent_cache_handles_oserror(tmp_path: Path) -> None:
    """write_agent_cache should silently handle OSError (read-only dir)."""
    read_only_dir = tmp_path / "readonly"
    read_only_dir.mkdir()
    read_only_dir.chmod(0o444)
    agents_by_host, _ = build_agents_by_host_from_tuples([("agent1", "local", "host-1")])
    try:
        # This should not raise -- OSError is caught internally
        write_agent_cache(read_only_dir, agents_by_host)
    finally:
