...
mng plugin add <name>        # Install a plugin (pip/uv install) [future]
mng plugin remove <name>     # Uninstall a plugin [future]
```
...
Called at various points in the execution of any `mng` command:
...
| `on_startup`               | Runs when `mng` starts up. Good for registering other callbacks. See [the `mng` API](./api.md) for more details on registration hooks.                |
...
| `on_shutdown`              | Runs when `mng` is shutting down. Good for cleaning up global state or resources.                                                                      |
...
Except for `on_host_collected` (which is called by any command that observes a host), these are mostly called during `mng create` and `mng destroy` operations:
