...
What counts as "activity" is highly configurable. Run `mng limit --help` [future] to see the available flags.
...
User input tracking requires either terminal access via `mng connect` or web access via `mng open` (which uses the [user_activity_tracking_via_web plugin](../core_plugins/user_activity_tracking_via_web.md), enabled by default).

`mng connect` tracks keystrokes in the terminal session and reports activity by writing JSON to `$MNG_HOST_DIR/activity/user`.

`mng open` opens the agent's URLs in a web browser. For web interfaces, the [user_activity_tracking_via_web plugin](../core_plugins/user_activity_tracking_via_web.md) injects JavaScript that tracks both mouse movements and keystrokes, reporting activity back to the agent in the same way.
...
Most agents should be configured to write to the special file `$MNG_HOST_DIR/agents/{agent_id}/activity/agent` whenever they produce output or are thinking.
...
touch "$MNG_HOST_DIR/activity/user"
```
...
```bash
TIME_MS=$(($(date +%s) * 1000))
printf '{\n  "time": %d\n}\n' "$TIME_MS" > "$MNG_HOST_DIR/activity/user"
```
