...
Many agents (Claude Code, Codex CLI, etc.) are terminal-based. This plugin automatically creates a web URL for any agent that doesn't already have one, allowing you to interact with the agent through your browser via `mng open`.
...
The token is generated when the agent is created and is included in the URL opened by `mng open`.
