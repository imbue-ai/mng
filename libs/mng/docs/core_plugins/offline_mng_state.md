# offline_mng_state Plugin [future]
...
Typically, `mng` queries each host directly to get the current status of its agents.
...
You can configure the plugin in your `mng` config file:

```toml
[plugins.offline_mng_state]
# Interval (in seconds) between automatic saves
save_interval_seconds = 300
# Storage backend: "local" or "s3"
storage_backend = "local"
# Local directory path (if using local backend), defaults to ~/.mng/plugin/offline_mng_state/<host_id>
local_directory = "/path/to/offline_state"
# S3 bucket name (if using s3 backend)
s3_bucket = "my-mng-offline-state-bucket"
