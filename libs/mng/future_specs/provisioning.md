...
Before any provisioning steps run, mng calls `agent.on_before_provisioning()`. This method allows agent types to validate that required preconditions are met before any actual provisioning work begins.
...
If validation fails, the method should raise a `PluginMngError` with a clear message explaining what is missing and how to fix it. This ensures that provisioning fails fast with actionable error messages rather than failing partway through after already making changes.
...
1. **Collection phase**: Before provisioning begins, mng calls `agent.get_provision_file_transfers()` to collect all file transfer requests.
2. **Validation phase**: For each transfer where `is_required=True`, mng verifies that `local_path` exists. If any required file is missing, provisioning fails with a clear error listing all missing files.
...
- **Project-specific files**: Transfer files referenced in `.mng/settings.toml` that aren't part of the work_dir
...
The default behavior is intended to make `mng` more usable--this way if something fails, the agent type can automatically fix it (rather than forcing the user to debug missing dependencies themselves).
...
After all provisioning steps have completed, mng calls `agent.on_after_provisioning()`. This method allows agent types to perform any finalization or verification steps after provisioning is done.
