...
A **ProviderInstance** is a configured endpoint that creates and manages [hosts](./host.md). Each provider instance is created by a [ProviderBackend](../imbue/mng/interfaces/provider_backend.py), which defines the type of infrastructure (Docker, Modal, local, etc.).
...
- **Stateless**: ProviderInstance stores no in-memory state. All host state is stored via provider-native mechanisms (Docker labels, Modal tags, local JSON files). This allows multiple mng instances to manage the same hosts.
...
A ProviderBackend is a parameterized factory. A ProviderInstance is a concrete, configured endpoint created by that factory. Users define provider instances in their mng settings:
...
ProviderInstance stores no in-memory state. All host state is stored via provider-native mechanisms (Docker labels, Modal tags, local JSON files). This allows multiple mng instances to manage the same hosts.
...
This ensures that mng can always reconstruct how to connect to a host by querying the provider, without needing to maintain a separate database of connection information.

### Build State Tracking [future]

In addition to tracking created hosts, providers need to track build state for hosts that failed during the build phase. This state should be stored on the local filesystem at `~/.mng/providers/<provider_name>/`.
...
When a provider is unreachable (e.g., no network connection, provider API is down), mng should fail quickly rather than hanging or timing out slowly. This is especially important when working offline.
...
mng should always cache the results of listing hosts from each provider. The cache should include:
...
2. **Offline Operation** [future]: When a provider is offline, cached results allow readonly operations like `mng list` to continue working
