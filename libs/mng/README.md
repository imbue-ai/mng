...
```bash
mng                  # launch claude on Modal (defaults: command=create, agent=claude, provider=modal, project=current dir)
mng --in local       # launch claude locally
mng my-task          # launch claude with a name on Modal
mng my-task codex    # launch codex instead of claude on Modal
mng -- --model opus  # launch pass any arguments to the agent running on Modal

# send an initial message so you don't have to wait around:
mng --no-connect --initial-message "Speed up one of my tests and make a PR on github"
...
**mng is fast:**
```bash
> time mng local-hello  --initial-message "Just say hello" --no-connect --in local
# (time results)

> time mng remote-hello --initial-message "Just say hello" --no-connect --in modal
# (time results)
...
```bash
mng create --in modal --no-connect --initial-message "just say 'hello'" --idle-timeout 60 -- --model sonnet
...
`mng` is built on open-source tools and standards (SSH, git, tmux, docker, etc.), and has [100's of additional plugins](http://imbue.com/mng/plugins) enable the latest AI coding workflows
...
- [`plugin`](docs/commands/secondary/plugin.md) [future]: Manage mng plugins
- [`config`](docs/commands/secondary/config.md): View and edit mng configuration

## How it works

You can interact with `mng` either via:

1. The terminal (run `mng --help` to learn more)
...
- `mng` is absurdly extensible--there are existing **[plugins](./docs/concepts/plugins.md)** for almost everything, and `mng` can even [dynamically generate new plugins]()
...
- agents store their status and logs in a standard location (default: `$MNG_STATE_DIR/<agent_id>/`)
