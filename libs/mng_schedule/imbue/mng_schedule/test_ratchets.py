...
def test_prevent_builtin_exception_raises() -> None:
    ...
    assert len(chunks) <= snapshot(7), PREVENT_BUILTIN_EXCEPTION_RAISES.format_failure(chunks)
...
def test_prevent_import_datetime() -> None:
    ...
    assert len(chunks) <= snapshot(1), PREVENT_IMPORT_DATETIME.format_failure(chunks)
...
def test_prevent_trailing_comments() -> None:
    ...
    assert len(chunks) <= snapshot(11), PREVENT_TRAILING_COMMENTS.format_failure(chunks)
...
def test_prevent_model_copy() -> None:
    ...
    assert len(chunks) <= snapshot(2), PREVENT_MODEL_COPY.format_failure(chunks)
...
def test_prevent_test_container_classes() -> None:
    ...
    assert len(chunks) <= snapshot(2), PREVENT_TEST_CONTAINER_CLASSES.format_failure(chunks)
...
def test_prevent_direct_subprocess_usage() -> None:
    ...
    assert len(chunks) <= snapshot(5), PREVENT_DIRECT_SUBPROCESS.format_failure(chunks)
...
def test_prevent_if_elif_without_else() -> None:
    ...
    assert len(chunks) <= snapshot(2), PREVENT_IF_ELIF_WITHOUT_ELSE.format_failure(chunks)


def test_prevent_inline_functions_in_non_test_code() -> None:
    chunks = find_inline_functions(_get_project_source_dir())
    assert len(chunks) <= snapshot(1), PREVENT_INLINE_FUNCTIONS.format_failure(chunks)
