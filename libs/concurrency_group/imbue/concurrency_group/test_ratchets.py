...
def test_prevent_getattr() -> None:
    ...
    assert len(chunks) <= snapshot(2), PREVENT_GETATTR.format_failure(chunks)
...
def test_prevent_trailing_comments() -> None:
    ...
    assert len(chunks) <= snapshot(14), PREVENT_TRAILING_COMMENTS.format_failure(chunks)
...
def test_prevent_bare_generic_types() -> None:
    ...
    assert len(chunks) <= snapshot(7), PREVENT_BARE_GENERIC_TYPES.format_failure(chunks)
...
def test_prevent_direct_subprocess_usage() -> None:
    ...
    assert len(chunks) <= snapshot(3), PREVENT_DIRECT_SUBPROCESS.format_failure(chunks)
...
def test_prevent_inline_functions_in_non_test_code() -> None:
    ...
    assert len(chunks) <= snapshot(5), PREVENT_INLINE_FUNCTIONS.format_failure(chunks)
...
def test_prevent_init_methods_in_non_exception_classes() -> None:
    ...
    assert len(chunks) <= snapshot(8), PREVENT_INIT_IN_NON_EXCEPTION_CLASSES.format_failure(chunks)
