...
class TestFinishedProcessCheck:
    """Tests for FinishedProcess.check() error handling."""

    def test_check_raises_process_timeout_error_when_timed_out(self) -> None:
    ...
    def test_check_raises_process_error_when_nonzero_exit(self) -> None:
        process = FinishedProcess(
            returncode=42,
    ...
    def test_check_timeout_takes_precedence_over_nonzero_exit(self) -> None:
        process = FinishedProcess(
            returncode=1,
            stdout="",
            stderr="",
    ...
    def test_check_preserves_is_output_already_logged_in_error(self) -> None:
        process = FinishedProcess(
            returncode=1,
            stdout="",
            stderr="",
...
class TestPartialOutputContainerWrite:
    """Tests for PartialOutputContainer.write() line splitting logic."""

    def test_write_accumulates_output_in_buffer(self) -> None:
    ...
    def test_write_calls_callback_on_complete_line_ending_with_newline(self) -> None:
    ...
    def test_write_calls_callback_on_complete_line_ending_with_carriage_return(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_does_not_call_callback_for_incomplete_line(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_accumulates_partial_line_across_writes(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_handles_multiple_lines_in_single_write(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_handles_mixed_complete_and_incomplete_lines(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_handles_empty_bytes(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_with_no_callback_just_accumulates(self) -> None:
    ...
    def test_write_handles_utf8_characters(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
    ...
    def test_write_handles_invalid_utf8_with_replacement(self) -> None:
        received_lines: list[str] = []
        container = PartialOutputContainer(on_complete_line=received_lines.append)
...
class TestIsTimeout:
    """Tests for _is_timeout() helper function."""

    def test_is_timeout_returns_false_when_timeout_is_none(self) -> None:
        assert _is_timeout(None) is False

    def test_is_timeout_returns_true_when_time_has_passed(self) -> None:
        past_time = time.time() - 10.0
        assert _is_timeout(past_time) is True

    def test_is_timeout_returns_false_when_time_has_not_passed(self) -> None:
        future_time = time.time() + 100.0
        assert _is_timeout(future_time) is False


class TestShutdownPopen:
    """Tests for _shutdown_popen() SIGTERM to SIGKILL escalation."""

    def test_shutdown_popen_terminates_process_with_sigterm(self) -> None:
    ...
    def test_shutdown_popen_returns_returncode_after_terminate(self) -> None:
...
class TestOutputGatherer:
    """Tests for OutputGatherer.gather_output() reading loop."""

    def test_gather_output_reads_from_stdout_and_stderr(self) -> None:
    ...
    def test_gather_output_stops_when_shutdown_event_is_set(self) -> None:
    ...
    def test_get_incomplete_lines_returns_partial_content(self) -> None:
    ...
    def test_gather_output_handles_none_reads(self) -> None:
        class NoneReader:
