...
from imbue.mng_claude_changeling.resources.conversation_db import main
...
def test_insert_creates_record(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        row = conn.execute(
            "SELECT conversation_id, tags, created_at FROM changeling_conversations WHERE conversation_id = ?",
            ("conv-1",),
        ).fetchone()
...
def test_insert_replaces_existing(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(str(db_path)) as conn:
        row = conn.execute(
            "SELECT tags, created_at FROM changeling_conversations WHERE conversation_id = ?",
            ("conv-1",),
        ).fetchone()
    assert row is not None
