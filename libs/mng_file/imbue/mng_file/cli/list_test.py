...
def test_get_field_value_returns_name() -> None:
    entry = FileEntry(
        name="test.txt", path="/test.txt", file_type="file", size=100, modified="2026-01-01", permissions="-rw-r--r--"
    )
...
def test_get_field_value_returns_path() -> None:
    entry = FileEntry(
        name="test.txt", path="/home/test.txt", file_type="file", size=100, modified=None, permissions=None
    )
    assert _get_field_value(entry, "path") == "/home/test.txt"


def test_get_field_value_returns_file_type() -> None:
    entry = FileEntry(name="d", path="/d", file_type="directory", size=None, modified=None, permissions=None)
    assert _get_field_value(entry, "file_type") == "directory"


def test_get_field_value_returns_permissions() -> None:
    entry = FileEntry(name="f", path="/f", file_type="file", size=0, modified=None, permissions="-rwxr-xr-x")
    assert _get_field_value(entry, "permissions") == "-rwxr-xr-x"


def test_get_field_value_returns_dash_for_none_permissions() -> None:
    entry = FileEntry(name="f", path="/f", file_type="file", size=0, modified=None, permissions=None)
    assert _get_field_value(entry, "permissions") == "-"


def test_get_field_value_returns_modified() -> None:
    entry = FileEntry(name="f", path="/f", file_type="file", size=0, modified="2026-03-21+12:00:00", permissions=None)
    assert _get_field_value(entry, "modified") == "2026-03-21+12:00:00"


def test_entry_to_field_mapping_returns_correct_mapping() -> None:
    entry = FileEntry(
        name="test.txt", path="/test.txt", file_type="file", size=1024, modified="2026-01-01", permissions="-rw-r--r--"
    )
...
def test_emit_list_result_human_with_entries(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    entries = [
        FileEntry(
            name="file.txt", path="/file.txt", file_type="file", size=100, modified="2026-01-01", permissions=None
        ),
    ]
...
def test_emit_list_result_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    entries = [
        FileEntry(name="a.txt", path="/a.txt", file_type="file", size=50, modified=None, permissions=None),
    ]
...
def test_emit_list_result_jsonl(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    entries = [
        FileEntry(name="a.txt", path="/a.txt", file_type="file", size=50, modified=None, permissions=None),
        FileEntry(name="b.txt", path="/b.txt", file_type="file", size=100, modified=None, permissions=None),
    ]
    _emit_list_result(entries, ("name",), output_opts)
    captured = capsys.readouterr()
...
def test_list_files_on_volume_returns_file_entries(tmp_path: Path) -> None:
    ...
    assert dir_entry.file_type == "directory"
