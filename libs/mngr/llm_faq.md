...
Features mentioned in this FAQ that are not yet implemented:
- TYPE_CHECKING guard in `interfaces/host.py` for `ProviderInstanceInterface` import
- `Host.get_provider_instance()` method (field exists but no accessor method)
- `mngr pair` command with TUI for disconnect/pause selection
- `mngr cleanup` command with interactive mode (`mngr gc` exists but is pure CLI)
- Interactive TUI for message enqueuing in `mngr create` (only `--message` flag currently exists)
