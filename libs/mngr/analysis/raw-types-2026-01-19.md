...
Description: In `libs/mngr/imbue/mngr/interfaces/host.py:346-358`, several git-related fields use plain strings:
...
1. **AgentInfo.type** - Change to `AgentTypeName` (api/list.py:43)
2. **CertifiedHostData.image** - Change to `ImageReference | None` (interfaces/data_types.py:185-188)
3. **HostInfo.name** - Change to `HostName` (interfaces/data_types.py:257)
4. **VolumeInfo.size_bytes** - Change to `SizeBytes` (interfaces/data_types.py:216)
5. **SnapshotInfo.size_bytes** - Change to `SizeBytes | None` (interfaces/data_types.py:201-204)
6. **HookDefinition.command** - Change to `CommandString` (config/data_types.py:70)
7. **Plugin interface methods** - Change `plugin_name: str` to `PluginName` (interfaces/host.py & interfaces/agent.py)
8. **Agent registry functions** - Change `agent_type: str` to `AgentTypeName` (agents/agent_registry.py)
9. **AgentInfo.idle_mode** - Change to `IdleMode | None` (api/list.py:58)
11. **ConnectionOptions.attach_command** - Change to `CommandString | None` (api/data_types.py:172-175)
12. **Config registry functions** - Change to typed parameters (config/plugin_registry.py & config/provider_registry.py)
13. **EnvVar.key** - Create `EnvVarName` type and use it (config/data_types.py:54-55)
14. **ModalProviderInstance.app_name** - Create appropriate type or use `NonEmptyStr` (providers/modal/instance.py:84)
15. **GitOptions branch fields** - Create `GitBranchName` type and use it (interfaces/host.py:346-358)
16. **Timeout/duration fields** - Create typed duration types like `Seconds(NonNegativeInt)` (multiple files)
17. **Resource counts** - Change CPU to `PositiveInt`, GPU to `NonNegativeInt` (interfaces/data_types.py:113, 123)
18. **recency_idx** - Change to `NonNegativeInt` (interfaces/data_types.py:205-208)

**Note**: Item #10 (NamedCommand.command) is already implemented.
