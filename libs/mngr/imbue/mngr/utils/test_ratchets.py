...
def test_prevent_raw_dict_model_copy() -> None:
    ...
    Use model_copy_update with to_update/field_ref instead:
    ...
    tools can find all usages of a field. See style guide "Type-safe model_copy_update"
    section for details.
    """
...
def test_prevent_model_copy_with_to_update_dict() -> None:
    """Prevent direct usage of model_copy(update=to_update_dict(...)).
    ...
    model_copy_update wraps model_copy + to_update_dict and is easier
    to detect with a ratchet test.
    """
    pattern = RegexPattern(r"\.model_copy\(\s*update\s*=\s*to_update_dict\(", multiline=True)
    chunks = check_regex_ratchet(_get_mngr_source_dir(), FileExtension(".py"), pattern, _THIS_FILE)

    assert len(chunks) <= snapshot(0), format_ratchet_failure_message(
        rule_name="model_copy with to_update_dict",
        rule_description=(
            "Do not call model_copy(update=to_update_dict(...)). "
            "Use model_copy_update instead: obj.model_copy_update(to_update(obj.field_ref().field, value)). "
            "See style guide 'Type-safe model_copy_update' section."
        ),
        chunks=chunks,
