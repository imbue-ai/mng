...
def test_prevent_getattr() -> None:
    chunks = check_ratchet_rule(PREVENT_GETATTR, _get_mngr_source_dir(), _SELF_EXCLUSION)
    assert len(chunks) <= snapshot(8), PREVENT_GETATTR.format_failure(chunks)


def test_prevent_setattr() -> None:
    chunks = check_ratchet_rule(PREVENT_SETATTR, _get_mngr_source_dir(), _SELF_EXCLUSION)
