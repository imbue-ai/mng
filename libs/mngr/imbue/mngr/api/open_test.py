...
# Collect URLs opened during tests instead of launching a real browser.
_intercepted_urls: list[str] = []


def _fake_browser_open(url: str) -> None:
    """Test-only browser opener that records URLs instead of launching a browser."""
    _intercepted_urls.append(url)


@pytest.fixture(autouse=True)
def _clear_intercepted_urls() -> None:
...
def test_open_agent_url_opens_browser(
    ...
    open_agent_url(agent=agent, is_wait=False, is_active=False, browser_open=_fake_browser_open)
...
def test_open_agent_url_wait_exits_when_stop_event_set(
    ...
    open_agent_url(agent=agent, is_wait=True, is_active=False, stop_event=stop_event, browser_open=_fake_browser_open)
...
def test_open_agent_url_active_records_activity(
    ...
    open_thread = threading.Thread(
        ...
        kwargs={
            ...
            "browser_open": _fake_browser_open,
        },
        daemon=True,
    )
    open_thread.start()
...
def test_resolve_agent_url_returns_first_url_when_no_type(
    local_provider: LocalProviderInstance,
    temp_host_dir: Path,
    temp_work_dir: Path,
) -> None:
    agent = create_test_base_agent(
        local_provider, temp_host_dir, temp_work_dir, reported_urls={"terminal": "https://example.com/ttyd"}
    )
...
def test_resolve_agent_url_shows_available_types_in_error(
    ...
    with pytest.raises(UserInputError, match="Available types: terminal"):
...
def test_open_agent_url_with_type_opens_typed_url(
    ...
    open_agent_url(agent=agent, is_wait=False, is_active=False, url_type="terminal", browser_open=_fake_browser_open)
