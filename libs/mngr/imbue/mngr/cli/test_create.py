...
def test_no_await_ready_creates_agent_in_background(
    ...
    with tmux_session_cleanup(session_name):
        ...
        # Use a longer timeout because background creation uses os.fork() and the
        # child process may take longer to create the tmux session under load
        # (especially during parallel test execution with xdist).
        wait_for(
            lambda: tmux_session_exists(session_name),
            timeout=10.0,
            error_message=f"Expected tmux session {session_name} to exist",
        ...
        wait_for(command_is_running, timeout=10.0, error_message="Expected sleep command to be running")
