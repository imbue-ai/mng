...
def test_run_pager_writes_to_stdout_when_not_interactive(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    # In test environment, is_interactive_terminal() returns False naturally
...
def test_run_pager_uses_subprocess_when_interactive(
    ...
    output_file = tmp_path / "pager_output_84627.txt"
...
def test_run_pager_falls_back_on_subprocess_error(
    ...
    def _raise_subprocess_error(*args: object, **kwargs: object) -> None:
...
def test_show_help_with_pager_formats_and_displays_help(
    ...
    # In test environment, run_pager writes directly to stdout
...
def test_help_option_callback_shows_help_and_exits() -> None:
    ...
    ctx = click.Context(test_cmd)
    real_param = click.Option(["-h", "--help"], is_flag=True)

    # ctx.exit(0) raises click.exceptions.Exit
    with pytest.raises(click.exceptions.Exit):
        help_option_callback(ctx, real_param, True)
...
def test_help_option_callback_does_nothing_when_value_false() -> None:
    ...
    result = help_option_callback(ctx, real_param, False)
    assert result is None
...
def test_help_option_callback_does_nothing_during_resilient_parsing() -> None:
    ...
    # Should return without raising SystemExit
    result = help_option_callback(ctx, real_param, True)
    assert result is None
