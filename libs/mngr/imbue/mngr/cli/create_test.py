...
@patch("imbue.mngr.cli.create.ensure_host_started")
@patch("imbue.mngr.cli.create.get_provider_instance")
def test_try_reuse_existing_agent_multiple_matches_uses_first(
    mock_get_provider: MagicMock,
    mock_ensure_host_started: MagicMock,
    mock_ensure_agent_started: MagicMock,
) -> None:
    """When multiple agents match, uses the first one and logs warning."""
    mngr_ctx = MagicMock()
    host_ref_1 = _make_host_ref(host_id=TEST_HOST_ID_1, host_name="host-1")
    host_ref_2 = _make_host_ref(host_id=TEST_HOST_ID_2, host_name="host-2")
    agent_ref_1 = _make_agent_ref(agent_id=TEST_AGENT_ID_1, agent_name="test-agent", host_id=TEST_HOST_ID_1)
    agent_ref_2 = _make_agent_ref(agent_id=TEST_AGENT_ID_2, agent_name="test-agent", host_id=TEST_HOST_ID_2)

    agent_and_host_loader = MagicMock(return_value={host_ref_1: [agent_ref_1], host_ref_2: [agent_ref_2]})

    # Setup mocks for first host (which should be used)
    mock_provider = MagicMock()
    mock_get_provider.return_value = mock_provider
    mock_host = MagicMock()
    mock_provider.get_host.return_value = mock_host
    ...
    # Should return successfully with the first agent
    assert result is not None
    agent, host = result
    assert agent == mock_agent
