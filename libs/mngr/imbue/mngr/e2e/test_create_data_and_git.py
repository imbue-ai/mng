...
def test_create_with_copy(e2e: E2eSession) -> None:
    ...
    expect(
        e2e.run(
            "mngr create my-task --copy --command 'sleep 99999' --no-ensure-clean",
...
def test_create_copy_with_existing_branch(e2e: E2eSession) -> None:
    ...
    expect(
        e2e.run(
            f"mngr create my-task --copy --branch {current_branch} --command 'sleep 99999' --no-ensure-clean",
...
def test_create_with_clone(e2e: E2eSession) -> None:
    ...
    expect(
        e2e.run(
            "mngr create my-task --clone --command 'sleep 99999' --no-ensure-clean",
