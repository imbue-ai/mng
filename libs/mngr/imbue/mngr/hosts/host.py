...
class Host(BaseHost, OnlineHostInterface):
    ...
    def _create_host_tmux_config(self) -> Path:
        ...
        lines = [
            ...
            "",
            "# Ctrl-x: Detach and stop the agent whose session this is",
            """bind -n C-x run-shell 'SESSION=$(tmux display-message -p "#{session_name}"); tmux detach-client -E "mngr stop --session $SESSION"'""",
            "",
    ...
    def start_agents(self, agent_ids: Sequence[AgentId]) -> None:
        ...
        - Adds a Ctrl-x binding to detach and stop the current agent
        """
