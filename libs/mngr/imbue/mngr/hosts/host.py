...
class Host(BaseHost, OnlineHostInterface):
            ...
            for agent_id in agent_ids:
                ...
                with log_span("Starting agent {} in tmux session {}", agent.name, session_name):
                    ...
                    combined_command = _build_start_agent_shell_command(
                        ...
                        tmux_socket_name=self.mngr_ctx.config.tmux_socket_name,
                    )
    ...
    def _collect_session_pids(self, session_name: str) -> list[str]:
        ...
        """
        all_pids: list[str] = []
        tmux_socket = self.mngr_ctx.config.tmux_socket_name
        list_panes_cmd = build_tmux_shell_cmd(
            tmux_socket, f"list-panes -s -t '{session_name}' -F '#{{pane_pid}}' 2>/dev/null || true"
        )
        result = self.execute_command(list_panes_cmd)
        if result.success and result.stdout.strip():
    ...
    def stop_agents(self, agent_ids: Sequence[AgentId], timeout_seconds: float = 5.0) -> None:
        ...
        with log_span("Stopping {} agent(s) with timeout={}s", len(agent_ids), timeout_seconds):
            ...
            for agent in current_agents:
                session_name = f"{self.mngr_ctx.config.prefix}{agent.name}"
                kill_cmd = build_tmux_shell_cmd(tmux_socket, f"kill-session -t '{session_name}' 2>/dev/null || true")
                self.execute_command(kill_cmd)
...
def _build_start_agent_shell_command(
    ...
    steps.append(
        build_tmux_shell_cmd(
            tmux_socket_name,
            f"set-option -t {shlex.quote(session_name)} default-command {shlex.quote(env_shell_cmd)}",
        )
    )

    # Send the agent command as literal keys, then Enter to execute
    steps.append(
        build_tmux_shell_cmd(tmux_socket_name, f"send-keys -t {shlex.quote(session_name)} -l {shlex.quote(command)}")
    )
    steps.append(build_tmux_shell_cmd(tmux_socket_name, f"send-keys -t {shlex.quote(session_name)} Enter"))
    ...
    for idx, named_cmd in enumerate(additional_commands):
        ...
        steps.append(
            build_tmux_shell_cmd(
                tmux_socket_name,
                f"send-keys -t {shlex.quote(window_target)} -l {shlex.quote(str(named_cmd.command))}",
            )
        )
        steps.append(build_tmux_shell_cmd(tmux_socket_name, f"send-keys -t {shlex.quote(window_target)} Enter"))

    # If we created additional windows, select the first window (the main agent)
    if additional_commands:
        steps.append(build_tmux_shell_cmd(tmux_socket_name, f"select-window -t {shlex.quote(session_name + ':0')}"))
    ...
    list_panes_cmd = build_tmux_shell_cmd(
        tmux_socket_name, f"list-panes -t {shlex.quote(session_name)} -F '#{{pane_pid}}' 2>/dev/null"
    )
    monitor_script = (
