...
def test_provision_agent_pass_env_vars(
    host_with_temp_dir: tuple[Host, Path], tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Test that pass_env_vars forwards environment variables from current shell."""
    host, temp_dir = host_with_temp_dir
    agent = _create_minimal_agent(host, temp_dir)

    # Set an environment variable in the current shell
    monkeypatch.setenv("MY_PASS_THROUGH_VAR", "passed_value_123")

    options = CreateAgentOptions(
        name=AgentName("pass-env-test"),
        agent_type=AgentTypeName("generic"),
        command=CommandString("sleep 1"),
        environment=AgentEnvironmentOptions(
            pass_env_vars=("MY_PASS_THROUGH_VAR",),
        ),
    )

    host.provision_agent(agent, options, host.mngr_ctx)

    # Check that the pass-through var appears in the env file
    env_path = temp_dir / "agents" / str(agent.id) / "env"
    content = env_path.read_text()
...
def test_provision_agent_pass_env_vars_missing_var(
    ...
    """Test that missing pass_env_vars are silently ignored."""
    host, temp_dir = host_with_temp_dir
    agent = _create_minimal_agent(host, temp_dir)
    ...
    options = CreateAgentOptions(
        name=AgentName("pass-env-missing-test"),
        agent_type=AgentTypeName("generic"),
        command=CommandString("sleep 1"),
        environment=AgentEnvironmentOptions(
            pass_env_vars=("NONEXISTENT_VAR",),
        ),
    )
    ...
    # Check that the missing var does not appear in the env file
    env_path = temp_dir / "agents" / str(agent.id) / "env"
    content = env_path.read_text()
