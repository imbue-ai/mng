...
class ModalProviderInstance(BaseProviderInstance):
    ...
    def _start_sshd_in_sandbox(
        ...
        logger.bind(ssh_user=ssh_user).debug("Configuring SSH keys in sandbox")
    ...
    def _setup_sandbox_ssh_and_create_host(
        ...
        logger.bind(ssh_host=ssh_host, ssh_port=ssh_port).debug("SSH endpoint available")

        # Add the host to our known_hosts file before waiting for sshd
        logger.bind(ssh_host=ssh_host, ssh_port=ssh_port).debug("Adding host to known_hosts")
    ...
    def create_host(
        ...
        logger.bind(app_name=self.app_name).debug("Getting Modal app")
        app = self._get_modal_app()

        # Create the sandbox
        logger.bind(timeout=config.timeout, cpu=config.cpu, memory_gb=config.memory).debug("Creating Modal sandbox")
        ...
        logger.bind(sandbox_id=sandbox.object_id).debug("Created Modal sandbox")
        ...
        logger.bind(host_id=str(host_id)).debug("Writing host record to volume")
    ...
    def stop_host(
        ...
        if sandbox:
            ...
            logger.bind(host_id=str(host_id)).debug("No sandbox found, may already be terminated")
    ...
    def start_host(
        ...
        logger.bind(host_id=str(host_id), snapshot_id=str(snapshot_id)).info("Restoring Modal sandbox from snapshot")
        ...
        logger.bind(image_id=modal_image_id).debug("Creating sandbox from snapshot image")
        ...
        logger.bind(sandbox_id=new_sandbox.object_id).info("Created sandbox from snapshot")
        ...
        logger.bind(host_id=str(host_id), host_name=str(host_name)).info("Restored Modal host from snapshot")
    ...
    def create_snapshot(
        ...
        """
        host_id = host.id if isinstance(host, HostInterface) else host
        logger.bind(host_id=str(host_id)).debug("Creating snapshot for Modal sandbox")
    ...
    def delete_snapshot(
        ...
        """
        host_id = host.id if isinstance(host, HostInterface) else host
        logger.bind(snapshot_id=str(snapshot_id), host_id=str(host_id)).debug("Deleting snapshot from Modal sandbox")
        ...
        logger.bind(snapshot_id=str(snapshot_id)).info("Deleted snapshot")
...
def _build_modal_secrets_from_env(
    ...
    logger.bind(count=len(secret_dict)).debug("Creating Modal secrets from environment variables")
