...
def test_mngr_enforce_stops_idle_modal_host(
    ...
    Creates a Modal agent with --idle-mode disabled (empty activity_sources,
    so enforce sees idle_seconds=inf which exceeds any timeout) and verifies:
    1. enforce --dry-run detects the idle violation without taking action
    2. enforce (non-dry-run) calls stop_host on the idle host
    3. The host remains alive (sleep 86400) so the activity_watcher's
       "no sessions" check doesn't interfere

    Uses --quiet to suppress stdout log messages that would mix with JSON.
    """
    ...
    # With disabled mode, activity_sources is empty. enforce respects
    # configured sources, so get_idle_seconds() checks no sources
