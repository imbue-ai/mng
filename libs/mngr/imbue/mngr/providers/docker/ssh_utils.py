"""SSH utilities for Docker provider.

Handles SSH key generation and management for Docker container access.
"""
...
def save_ssh_keypair(key_dir: Path, key_name: str = "docker_ssh_key") -> tuple[Path, Path]:
...
def load_or_create_ssh_keypair(key_dir: Path, key_name: str = "docker_ssh_key") -> tuple[Path, str]:
...
def load_or_create_host_keypair(key_dir: Path, key_name: str = "host_key") -> tuple[Path, str]:
    ...
    This key is used as the SSH host key for all Docker containers, allowing us
