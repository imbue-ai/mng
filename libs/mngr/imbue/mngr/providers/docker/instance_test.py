...
@pytest.mark.docker_sdk
def test_docker_client_caches_failure(temp_mngr_ctx: MngrContext) -> None:
    """Repeated access to _docker_client raises the same cached error without retrying."""
    provider = make_offline_docker_provider(temp_mngr_ctx)

    with pytest.raises(ProviderUnavailableError) as first:
        _ = provider._docker_client
    with pytest.raises(ProviderUnavailableError) as second:
        _ = provider._docker_client
...
@pytest.mark.docker_sdk
def test_list_volumes_returns_empty_when_daemon_offline(temp_mngr_ctx: MngrContext) -> None:
    """list_volumes returns [] when Docker is unreachable, avoiding noisy errors during GC."""
    provider = make_offline_docker_provider(temp_mngr_ctx)
    # Trigger the cached failure
    with pytest.raises(ProviderUnavailableError):
        _ = provider._docker_client
    # list_volumes should return [] silently
    assert provider.list_volumes() == []


@pytest.mark.docker_sdk
def test_repeated_discover_hosts_does_not_retry_connection(temp_mngr_ctx: MngrContext) -> None:
    """Calling discover_hosts multiple times after a failure returns [] without retrying."""
    provider = make_offline_docker_provider(temp_mngr_ctx)
    cg = temp_mngr_ctx.concurrency_group

    # First call triggers the connection attempt and caches the failure
    assert provider.discover_hosts(cg=cg) == []
    # Subsequent calls should return [] immediately via the fast path
    assert provider.discover_hosts(cg=cg) == []
    assert provider.discover_hosts(cg=cg) == []
