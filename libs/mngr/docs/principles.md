...
Features not yet fully implemented that would strengthen adherence to these principles:

**Safe:**
- `snapshot` command - create/list/restore agent snapshots for recovery
- `stop`/`start` commands - explicit host lifecycle control
- `clone`/`migrate` commands - safely copy/move agents between hosts
- Automatic idle detection enforcement with host suspension
- Devcontainer lifecycle hooks for safe environment setup

**Direct:**
- `limit` command - explicit resource limit configuration
- `open` command - direct access to agent URLs
- Docker/AWS provider implementations for transparent multi-cloud support

**Immediate:**
- `push`/`pair` commands - real-time file synchronization
- Connection pooling optimizations for remote operations

**Personal:**
- `ask` command - local AI help without external data sharing
- Explicit data collection configuration UI
