...
`mngr connect` tracks keystrokes in the terminal session and reports activity to the agent (by calling `cat user_activity > $MNGR_HOST_DIR/activity/user` inside the host).

`mngr open` opens the agent's URLs in a web browser. For web interfaces, the [user_activity_tracking_via_web plugin](../core_plugins/user_activity_tracking_via_web.md) injects JavaScript that tracks both mouse movements and keystrokes, reporting activity back to the agent in the same way (via `cat user_activity > $MNGR_HOST_DIR/activity/user`).
