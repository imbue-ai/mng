"""Unit tests for the mngr_elena_code plugin."""
...
from imbue.mngr_elena_code.plugin import ELENA_SYSTEM_PROMPT
from imbue.mngr_elena_code.plugin import ElenaCodeAgent
from imbue.mngr_elena_code.plugin import _merge_system_prompt_into_args
from imbue.mngr_elena_code.plugin import register_agent_type
...
def _make_elena_agent(tmp_path: Path) -> tuple[ElenaCodeAgent, OnlineHostInterface]:
    ...
    config = MngrConfig(default_host_dir=tmp_path / "host")
    mngr_ctx = MngrContext.model_construct(
        config=config,
        pm=pm,
