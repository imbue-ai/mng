...
def test_prevent_trailing_comments() -> None:
    ...
    assert len(chunks) <= snapshot(12), PREVENT_TRAILING_COMMENTS.format_failure(chunks)
...
def test_prevent_inline_functions_in_non_test_code() -> None:
    ...
    assert len(chunks) <= snapshot(2), PREVENT_INLINE_FUNCTIONS.format_failure(chunks)
...
def test_prevent_init_methods_in_non_exception_classes() -> None:
    ...
    assert len(chunks) <= snapshot(1), PREVENT_INIT_IN_NON_EXCEPTION_CLASSES.format_failure(chunks)
