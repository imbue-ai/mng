...
def test_build_command_map_returns_builtins_with_no_user_config() -> None:
    commands = _build_command_map(_make_mngr_ctx_with_plugins())  # ty: ignore[invalid-argument-type]
...
def test_build_command_map_user_command_overrides_builtin() -> None:
    ...
    commands = _build_command_map(mngr_ctx)  # ty: ignore[invalid-argument-type]
...
def test_build_command_map_disabled_command_is_excluded() -> None:
    ...
    assert "d" not in _build_command_map(mngr_ctx)  # ty: ignore[invalid-argument-type]


def test_load_user_commands_no_kanpan_plugin_returns_empty() -> None:
    assert _load_user_commands(_make_mngr_ctx_with_plugins()) == {}  # ty: ignore[invalid-argument-type]
...
def test_load_user_commands_handles_dict_values() -> None:
    ...
    commands = _load_user_commands(mngr_ctx)  # ty: ignore[invalid-argument-type]
...
def test_load_user_columns_no_config_returns_empty() -> None:
    assert _load_user_columns(_make_mngr_ctx_with_plugins()) == {}  # ty: ignore[invalid-argument-type]
...
def test_load_user_columns_handles_dict_values() -> None:
    ...
    mngr_ctx = _make_mngr_ctx_with_plugins({PluginName("kanpan"): config})
    columns = _load_user_columns(mngr_ctx)  # ty: ignore[invalid-argument-type]
