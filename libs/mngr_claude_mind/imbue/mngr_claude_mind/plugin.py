...
from imbue.mngr_llm.plugin import set_llm_model_env_var
from imbue.mngr_llm.plugin import set_uv_tool_env_vars
from imbue.mngr_llm.provisioning import check_llm_toolchain
...
from imbue.mngr_llm.provisioning import create_mind_conversations_table
...
from imbue.mngr_llm.settings import load_from_host
from imbue.mngr_mind.data_types import SOURCE_COMMON_TRANSCRIPT
...
class ClaudeMindAgent(ClaudeAgent):
    ...
    During provisioning:
    - Installs the llm toolchain (via mngr_llm plugin)
    - Provisions the link_skills.sh script (via mngr_mind plugin)
    ...
    - Event watcher (sends new events to primary role agent via mngr message)
    - Web server (main web interface with conversation selector and agent list)
    - Observer (runs mngr observe writing agent state events to the agent state directory)
    """
    ...
    def modify_env_vars(
        ...
        """Set UV tool dirs and MNGR_LLM_MODEL for per-agent tool isolation and chat."""
    ...
    def provision(
        ...
        1. Per-agent mngr installation (via mngr_recursive, before super())
        2. Settings loading from minds.toml
        3. llm + plugin installation (via mngr_llm)
        4. link_skills.sh script (via mngr_mind)
        ...
        9. Supporting service scripts and chat utilities (via mngr_llm)
        10. LLM tool scripts for conversation context (via mngr_llm)
        11. Per-role memory directory setup
        """

        provision_mngr_for_agent(agent=self, host=host, mngr_ctx=mngr_ctx)
    ...
    # bypassPermissionsModeAccepted: handled by skipDangerousModePermissionPrompt
    # in settings.json (generated by _build_settings_json). No per-agent override needed.
