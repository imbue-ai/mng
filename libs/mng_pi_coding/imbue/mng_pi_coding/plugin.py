...
def _install_pi(host: OnlineHostInterface) -> None:
    ...
    if not result.success:
        raise PluginMngError(f"Failed to install pi. stderr: {result.stderr}")
...
class PiCodingAgent(BaseAgent):
    ...
    def get_pi_config_dir(self) -> Path:
        ...
        is set. Located at $MNG_AGENT_STATE_DIR/plugin/pi_coding/.
        """
    ...
    def provision(
        ...
        if config.check_installation:
            ...
            if is_installed:
                ...
                if host.is_local and not mng_ctx.is_auto_approve:
                    raise PluginMngError(f"pi is not installed. Please install it with:\n  {install_hint}")
                elif not host.is_local and not mng_ctx.config.is_remote_agent_installation_allowed:
