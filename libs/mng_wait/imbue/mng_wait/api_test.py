...
from imbue.mng_wait.api import _build_agent_resolved_target
from imbue.mng_wait.api import _build_host_resolved_target
...
def test_build_agent_resolved_target_finds_agent_by_id(temp_mng_ctx: MngContext) -> None:
    agents_by_host, host_ref, agent_ref = _make_agents_by_host(agent_name="my-agent")
    agent_id_str = str(agent_ref.agent_id)
    result = _build_agent_resolved_target(agent_id_str, agents_by_host, temp_mng_ctx)
...
def test_build_agent_resolved_target_raises_when_not_found(temp_mng_ctx: MngContext) -> None:
    ...
    with pytest.raises(UserInputError):
        _build_agent_resolved_target(nonexistent_agent_id, agents_by_host, temp_mng_ctx)


# === _build_host_resolved_target ===


def test_build_host_resolved_target_finds_host_by_id(temp_mng_ctx: MngContext) -> None:
...
def test_build_host_resolved_target_raises_when_not_found(temp_mng_ctx: MngContext) -> None:
    agents_by_host, _host_ref, _agent_ref = _make_agents_by_host()
    all_hosts = list(agents_by_host.keys())
    nonexistent_host_id = str(HostId.generate())
    with pytest.raises(UserInputError):
        _build_host_resolved_target(nonexistent_host_id, all_hosts, temp_mng_ctx)


def test_build_host_resolved_target_handles_invalid_id_format(temp_mng_ctx: MngContext) -> None:
    """Ensure a host-prefixed name with invalid UUID format raises UserInputError, not InvalidRandomIdError."""
    agents_by_host, _host_ref, _agent_ref = _make_agents_by_host()
    all_hosts = list(agents_by_host.keys())
    with pytest.raises(UserInputError):
        _build_host_resolved_target("host-myserver", all_hosts, temp_mng_ctx)
...
def test_resolve_by_name_finds_agent(temp_mng_ctx: MngContext) -> None:
    agents_by_host, host_ref, agent_ref = _make_agents_by_host(agent_name="my-agent")
    all_hosts = list(agents_by_host.keys())
    result = _resolve_by_name("my-agent", agents_by_host, all_hosts, temp_mng_ctx)
    assert result.target.target_type == WaitTargetType.AGENT
    assert result.agent_id == agent_ref.agent_id
    assert result.host_id == host_ref.host_id
...
def test_resolve_by_name_finds_host(temp_mng_ctx: MngContext) -> None:
    ...
    all_hosts = list(agents_by_host.keys())
    result = _resolve_by_name("my-host", agents_by_host, all_hosts, temp_mng_ctx)
    assert result.target.target_type == WaitTargetType.HOST
...
def test_resolve_by_name_raises_when_ambiguous(temp_mng_ctx: MngContext) -> None:
    ...
    with pytest.raises(UserInputError, match="matches both"):
        _resolve_by_name(shared_name, agents_by_host, all_hosts, temp_mng_ctx)
...
def test_resolve_by_name_raises_when_not_found(temp_mng_ctx: MngContext) -> None:
    ...
    with pytest.raises(UserInputError, match="No agent or host found"):
        _resolve_by_name("nonexistent", agents_by_host, all_hosts, temp_mng_ctx)
...
def test_resolve_by_name_raises_when_multiple_agents(temp_mng_ctx: MngContext) -> None:
    ...
    with pytest.raises(UserInputError, match="Multiple"):
        _resolve_by_name("dup-agent", agents_by_host, all_hosts, temp_mng_ctx)
...
def test_resolve_by_name_raises_when_multiple_hosts(temp_mng_ctx: MngContext) -> None:
    ...
    with pytest.raises(UserInputError, match="Multiple"):
        _resolve_by_name("dup-host", agents_by_host, all_hosts, temp_mng_ctx)
