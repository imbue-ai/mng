...
from imbue.minds.errors import GitCommitError
from imbue.minds.errors import GitInitError
...
class DeploymentResult(FrozenModel):
    """Result of a successful mind deployment."""
    ...
    login_url: str = Field(description="One-time login URL for accessing the mind")


class UpdateResult(FrozenModel):
    """Result of a successful local mind update."""
...
class MngNotFoundError(MindError):
    """Raised when the mng binary cannot be found on PATH."""

    ...


class MngCreateError(MindError):
    """Raised when mng create fails."""

    ...


class AgentIdLookupError(MindError):
...
def deploy_mind(
    ...
    """Deploy a mind by creating an mng agent.

    The mind_dir is the permanent mind directory (e.g.
    ~/.minds/<agent-id>/) containing the prepared repo. The caller
    ...
    runs directly in mind_dir.

    For remote deployments (Modal, Docker), the code is copied from
    mind_dir to the remote host via --source-path. The caller
    is responsible for cleaning up mind_dir afterwards.

    This function:
    1. Verifies mng is available and no agent with this name exists
    2. Creates an mng agent via `mng create --id <id> --type <type> --label mind=true`
    ...
    """
    with log_span("Deploying mind '{}' via provider '{}'", agent_name, provider.value):
...
def _create_mng_agent(
    ...
    """Create an mng agent from the mind's repo directory.

    The agent_id is passed via --id so mng uses the same ID as the
    mind directory name (~/.minds/<agent-id>/).
    ...
    with log_span("Creating mng agent '{}' via provider '{}'", agent_name, provider.value):
        ...
        if provider == DeploymentProvider.LOCAL:
            ...
            # permanent mind directory.
            mng_command.append("--in-place")
        else:
            # Remote: use the mind directory as source and deploy
...
def update_local(
    ...
    """Update an existing locally-deployed mind.
    ...
    """
    with log_span("Updating mind '{}' locally", agent_name):
        _verify_mng_available()
