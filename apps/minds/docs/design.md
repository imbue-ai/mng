...
Each mind has its own repo stored at `~/.minds/<agent-id>/`. This repo is created by cloning from a git remote when the user creates a mind via the forwarding server. The agent runs directly in this directory (via `mngr create --in-place`) and should make commits there if it changes anything.
...
4. Runs `mngr create --type <type> --id <id> --in-place --label mind=true` to start the agent
