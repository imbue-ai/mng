...
4. We prepare a temporary repo (either by cloning or by creating an empty git repo with .mng/settings.toml). For local deployment, we move it to `~/.changelings/<agent-id>/` and run `mng create --name <name> --no-connect --in-place -t entrypoint --label changeling=true`. For remote deployment, we run `mng create --name <name> --no-connect --in <provider> --source-path <temp-dir> -t entrypoint --label changeling=true` and clean up the temp dir afterwards.
