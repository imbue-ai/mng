...
4. We prepare a temporary repo (either by cloning or by creating an empty git repo with .mng/settings.toml). For local deployment, we run `mng create --in-place -t entrypoint --label changeling=true` from the temp dir, then move it to `~/.changelings/<agent-id>/` (using the mng-assigned agent ID). For remote deployment, we run `mng create --in <provider> --source-path <temp-dir> -t entrypoint --label changeling=true` and clean up the temp dir afterwards.
