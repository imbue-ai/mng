...
# -- Sibling server passthrough tests --


def test_service_worker_js_passes_through_sibling_server_paths() -> None:
    """Verify the SW does not rewrite paths to other servers of the same agent."""
    js = generate_service_worker_js(_TEST_AGENT, _TEST_SERVER)
    assert f"const AGENT_PREFIX = '/agents/{_TEST_AGENT}'" in js
    assert "AGENT_PREFIX" in js


def test_websocket_shim_passes_through_sibling_server_paths() -> None:
    """Verify the WS shim does not rewrite WebSocket URLs to sibling servers."""
    js = generate_websocket_shim_js(_TEST_AGENT, _TEST_SERVER)
    assert "AGENT_PREFIX" in js
    assert f"var AGENT_PREFIX = '/agents/{_TEST_AGENT}'" in js


def test_rewrite_absolute_paths_preserves_sibling_server_paths() -> None:
    """Verify that paths to sibling servers are not rewritten."""
    sibling_path = f"/agents/{_TEST_AGENT}/{_TEST_SERVER_2}/some/page"
    html = f'<iframe src="{sibling_path}"></iframe>'
    result = rewrite_absolute_paths_in_html(
        html_content=html,
        agent_id=_TEST_AGENT,
        server_name=_TEST_SERVER,
    )
    assert sibling_path in result


def test_rewrite_absolute_paths_preserves_sibling_server_root() -> None:
    """Verify that root paths to sibling servers are not rewritten."""
    sibling_path = f"/agents/{_TEST_AGENT}/{_TEST_SERVER_2}/"
    html = f'<a href="{sibling_path}">link</a>'
    result = rewrite_absolute_paths_in_html(
        html_content=html,
        agent_id=_TEST_AGENT,
        server_name=_TEST_SERVER,
...
def test_rewrite_absolute_paths_still_rewrites_non_agent_paths() -> None:
    """Verify that normal absolute paths are still rewritten even with sibling passthrough."""
    html = '<a href="/style.css">link</a>'
    result = rewrite_absolute_paths_in_html(
        html_content=html,
        agent_id=_TEST_AGENT,
        server_name=_TEST_SERVER,
    )
    assert f"/agents/{_TEST_AGENT}/{_TEST_SERVER}/style.css" in result
