...
def test_deploy_fails_for_invalid_git_url() -> None:
    runner = CliRunner()
    result = runner.invoke(cli, ["deploy", "/nonexistent/repo/path"])
...
def test_deploy_fails_when_no_changeling_toml(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["deploy", str(repo_dir)])
...
def test_deploy_rejects_modal_provider(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["deploy", str(repo_dir)], input="test-bot\n2\nN\n")
...
def test_deploy_shows_prompts(tmp_path: Path) -> None:
    ...
    runner = CliRunner()
    result = runner.invoke(cli, ["deploy", str(repo_dir)], input="my-agent\n2\nN\n")
...
def test_deploy_name_flag_skips_prompt(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["deploy", str(repo_dir), "--name", "my-custom-name", "--provider", "modal", "--no-self-deploy"],
    )
...
def test_deploy_provider_flag_skips_prompt(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["deploy", str(repo_dir), "--provider", "modal", "--no-self-deploy"],
        input="test-bot\n",
    )
...
def test_deploy_self_deploy_flag_skips_prompt(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["deploy", str(repo_dir), "--no-self-deploy", "--provider", "modal"],
        input="test-bot\n",
    )
...
def test_deploy_all_flags_skip_all_prompts(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["deploy", str(repo_dir), "--name", "bot", "--provider", "modal", "--no-self-deploy"],
    )
...
def test_deploy_loads_agent_type_zygote(tmp_path: Path) -> None:
    ...
    result = runner.invoke(
        cli,
        ["deploy", str(repo_dir), "--name", "test-elena", "--provider", "modal", "--no-self-deploy"],
    )
