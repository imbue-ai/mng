...
- **changeling**: a persistent `mng` agent that serves a web interface and is conversational. Each changeling is identified by its `AgentId` (the standard mng agent identifier) and runs its own web server.
