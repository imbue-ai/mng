...
- **changeling**: a collection of persistent mng agents (called **role agents**) that serve a web interface and support chat input/output. The role agents coordinate through shared event streams in a common git repo. Each changeling is identified by its `AgentId` and is labeled with `changeling=true` for discovery via `mng list`. For local deployments, the changeling has a repo directory at `~/.changelings/<agent-id>/` containing a `.mng/settings.toml` with an "entrypoint" create template. For remote deployments (Modal, Docker), a temporary repo is prepared and the code is copied to the remote host via `mng create --in <provider>`.
