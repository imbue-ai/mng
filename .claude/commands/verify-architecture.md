...
Read the diff validation prompt from `.claude/skills/autofix/validate-diff.md`. Spawn an Agent subagent (`subagent_type: "general-purpose"`, `model: "haiku"`) with that prompt, providing the base branch name and the problem description from Phase 1.
...
Read the subagent prompt from `.claude/skills/verify-architecture/analyze-architecture.md`. Spawn a single Agent subagent (`subagent_type: "general-purpose"`, leaving model as default) with that prompt, prepending:
