...
allowed-tools: Bash(git rev-parse *), Bash(git diff *), Bash(git log *), Bash(git show *), Bash(git ls-tree *), Bash(ls *), Bash(find *), Bash(grep *), Bash(echo "${GIT_BASE_BRANCH:-main}"), Read, Agent, AskUserQuestion
---
...
Spawn an Agent (`subagent_type: "general-purpose"`, `model: "haiku"`) and tell it to read `.claude/skills/autofix/validate-diff.md` and follow its instructions. Provide the base branch name and the problem description from Phase 1.
...
Spawn a single Agent (`subagent_type: "general-purpose"`, leaving model as default) and tell it to read `.claude/skills/verify-architecture/analyze-architecture.md` and follow its instructions. Provide:
